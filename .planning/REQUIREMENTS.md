...
- [ ] **VIZ-04**: The product exposes four top-level views — Story, Characters, Evidence, Advanced — with Story coordinating Episode Overview and Event Timeline, Characters coordinating Character Network and Local Neighborhood, Evidence hosting Evidence Chain and temporary Answer Graph, and Advanced retaining the Full Graph/debug explorer. Raw Neo4j relation names remain hidden outside explicit debug mode.
- [ ] **VIZ-05**: Desktop uses top tabs; mobile mirrors the same hierarchy with horizontally scrollable top tabs and a half/full-height Inspector bottom sheet. Narrow screens never squeeze graph, timeline, and Inspector simultaneously, and selection synchronizes graph, timeline, and Inspector without relayout.
- [ ] **VIZ-06**: Semantic expansion is server-allowlisted (`family`, `work`, `conflict`, `episode_events`, `clues`, `locations`, `evidence`), spoiler-safe, targets 8–12 additions with hard max 25, exposes no hidden totals/future hints, and supports Collapse, Undo, and Reset.
