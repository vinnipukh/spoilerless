...
- [ ] **VIZ-03**: Episode Overview defaults to 12–28 nodes (hard max 40), prefers fewer than 35 edges (hard max 60), shows no persistent procedural labels, omits `PARTICIPATED_IN`/`OCCURRED_IN`, and uses safe-boundary `display_tier` editorial importance rather than full-graph degree. Fixed S01E01 and cumulative S01E02 snapshots compare variant A (characters + major Events) with variant B (character-led graph + timeline Events) before the production default is selected.
