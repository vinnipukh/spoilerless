...
## Current State (v1.4 Security Hardening shipped 2026-08-20 — extends v1.3)
...
- **Security-hardened (Phase 11 — audit remediation P0/P1, 8 plans green, 2026-08-20):** single fail-closed `resolve_effective_boundary` (`spoilerless/app/api/boundary.py`) gating every spoiler-sensitive read (anonymous fixed at 1, no-record fail-closed, clamped `min(requested, view, watched)` + persisted-episode 422); candidate ingest server-derives `visible_from_order` + existence checks + rate-limit + `invalidate_series` + pagination; trusted proxy + fail-closed rate limiting (`render.yaml --proxy-headers --forwarded-allow-ips` + `spoilerless/app/services/rate_limit.py` fail-closed 503 when `ENVIRONMENT=production` + `RATE_LIMIT_FAIL_OPEN=false`), `request.client is None` guard (BUG-BE-02); SSRF-hardened `base_url` via `ipaddress` blocklist (loopback/private/link-local/metadata, decimal/hex, `localhost`, trailing-dot — `spoilerless/app/domain/settings.py`); LLM cost caps — process-wide `asyncio.Semaphore(llm_max_concurrent_generations=4)` (`spoilerless/app/services/chat.py`) + per-round tool-call cap 8 (`spoilerless/app/retrieval/pipeline.py`) + `operations max_length` 20/50; `BodySizeLimitMiddleware` 413 (`max_body_size_bytes=1 MiB`) + docs-off in prod (`ENVIRONMENT=production` disables `/docs`/`/redoc`/`/openapi.json`) + `TrustedHostMiddleware` + `warn_if_open_signup`; CSP/security headers on API (`spoilerless/app/main.py`) and Vercel shell (`frontend/vercel.json`); log sanitization (validation `input`/`ctx` stripped — `spoilerless/app/core/errors.py`); delimiter neutralization (`spoilerless/app/retrieval/context.py` + `pipeline.py _neutralize_answer_delimiters`); bounded viz cache (`spoilerless/app/cache/graph_cache.py` `FOCUS_SET_CAP=64`); revert allowlist + ownership fail-closed (stored `None` requires admin) + `POST /change-sets/{id}/revert` admin-gated (`spoilerless/app/revisions/__init__.py` + `spoilerless/app/api/change_set.py`); `useWatchProgress` series-switch hydration + `apiFetch` bodyless `Content-Type` fixes; dynamic `run_doc_verification.py` + deleted superseded scripts (QUAL-01); `_propose_changeset_executor` → `ChangeSetService.propose_via_tool` (QUAL-02). `SECURITY_ATTACK_SURFACE.md` refreshed (route table + middleware + 9 grep markers), `SECURITY_TEST_PLAN.md` 33 × `[x]`, `docs/PROBLEMS.md` TWENTY-SECOND PASS, 8/8 summaries verified.
...
- ~24,000+ LOC across `spoilerless/app` and `frontend/src`. Python 3.13 + FastAPI + Pydantic v2 + Neo4j async driver; React 19 + TypeScript + Vite + Cytoscape.js; `uv` for Python packaging. Backend 142+ offline tests green; frontend 400 tests; coverage audit 98/98 source ids.
...
- ✓ Single fail-closed boundary resolver across all reads (graph/episodes/candidates/notes/custom/revisions/export/visualization/expand/path) — Phase 11 (SEC-01, SEC-BE-001)
- ✓ Anonymous clamp + auth/persist-validate reads — authenticated readers without progress fail-closed to 1, persisted-episode validation 422 — Phase 11 (SEC-02, SEC-BE-002 + SEC-ADV-003)
- ✓ Candidate ingest hardening — server-derived visible_from_order + subject/object/episode existence + ingest rate-limit + cache invalidation + pagination — Phase 11 (SEC-03, SEC-BE-003 + SEC-ADV-001/002)
- ✓ Trusted proxy + per-IP limits — Render `--proxy-headers --forwarded-allow-ips` (restricted CIDR, never `*`), XFF cannot spoof when untrusted — Phase 11 (SEC-04, SEC-BE-004/SEC-DOS-003)
- ✓ Fail-closed rate limiting — Redis outage in production (`ENVIRONMENT=production`, `RATE_LIMIT_FAIL_OPEN=false`) returns 503 `rate_limit_unavailable`, dev no-op preserved — Phase 11 (SEC-05, SEC-DOS-001)
- ✓ LLM cost controls — global semaphore 4 + per-round tool-call cap 8 + per-turn changeset op cap 20/50 + `warn_if_open_signup` — Phase 11 (SEC-06, SEC-DOS-002)
- ✓ SSRF hardening for BYOK + stored `base_url` — `ipaddress` blocklist (loopback/private/link-local/metadata, decimal/hex, `localhost`, trailing-dot), redirect off — Phase 11 (SEC-07, SEC-LLM-001/002)
- ✓ Request body-size limit 413 `payload_too_large` (1 MiB, pure ASGI, header + chunked) — Phase 11 (SEC-08, SEC-DOS-004)
- ✓ Docs off in production — FastAPI `docs_url=None` when `ENVIRONMENT=production` — Phase 11 (SEC-09, SEC-INF-003)
- ✓ CSP/security headers on Vercel shell + `index.html` meta fallback — mirrors backend `_SECURITY_HEADERS` + HSTS/nosniff/DENY — Phase 11 (SEC-10, SEC-FE-001)
- ✓ Log sanitization — validation errors log `loc/type/msg/code` only, never `input`/`ctx` with raw values — Phase 11 (SEC-11, SEC-LOG-001)
- ✓ P1 hardening — output guard/delimiter neutralization + bounded focus-set cache (64) + `operations` cap + revert label allowlist + fail-closed reversion ownership (`None` → admin) + ChangeSet revert admin gate + `useWatchProgress` hydration + `apiFetch` Content-Type + dynamic `run_doc_verification.py` (QUAL-01) + executor decoupling (QUAL-02) — Phase 11 (SEC-12)
...
| Centralized `resolve_effective_boundary` as single fail-closed seam (11-01/11-02) | Every spoiler-sensitive read must be server-clamped; anonymous and no-record must both be 1 — audit SEC-BE-001/002 | ✓ Good — `spoilerless/app/api/boundary.py` (66 lines) is the only resolver; `series.py`, `candidates.py`, `user_content.py`, `revisions.py` all delegate; persisted-episode 422; anonymous `visible_until_order=999` → 1 verified in `test_security_boundary.py` |
| Fail-closed rate limiting + trusted proxy (11-04) | Site-global login bucket and fail-open outage are product-fatal availability failures | ✓ Good — `spoilerless/app/services/rate_limit.py` now fail-closed 503 when `redis_url` set + `environment==production` + `rate_limit_fail_open is False`; `rate_limit_identifier` guards `request.client is None` (BUG-BE-02); `render.yaml` adds restricted `--forwarded-allow-ips` CIDRs, XFF not trusted when direct peer untrusted |
| SSRF-hardened `base_url` + LLM cost caps (11-05) | Shared stored `base_url` is an authenticated SSRF primitive; unbounded tool rounds farm operator wallet | ✓ Good — `spoilerless/app/domain/settings.py: _validate_base_url` blocklist via `ipaddress` (loopback/private/link-local/CGNAT/metadata, decimal/hex, `localhost`, trailing-dot) gated on `environment==production`; `spoilerless/app/services/chat.py` process-wide `asyncio.Semaphore(4)` + `spoilerless/app/retrieval/pipeline.py` per-round cap 8 + `operations` caps 20/50; `warn_if_open_signup` emits loud startup warning |
| Body-size + docs-off + log sanitization + delimiter + bounded cache + revert gating (11-06..11-08) | Unbounded bodies, open prod docs, raw `input`/`ctx` in logs, unbounded cache cardinality, and permissive revert allowlist are P0/P1 findings | ✓ Good — `BodySizeLimitMiddleware` 413 `payload_too_large` (1 MiB, header + streaming), docs disabled via `_docs_kwargs` when `environment==production`, validation logs strip `input`/`ctx` (`spoilerless/app/core/errors.py`), `spoilerless/app/retrieval/context.py` + `pipeline.py` neutralize exact `<CONTEXT_SECTIONS>` tags, `cache/graph_cache.py` bounds per-series focus sets to 64, `revisions/__init__.py` allowlist + `stored_owner is None` fail-closed + `POST /change-sets/{id}/revert` admin-gated via `RequireAdminDependency`; `run_doc_verification.py` now `Path(__file__).resolve().parent` (QUAL-01) + `ChangeSetService.propose_via_tool` extraction (QUAL-02) |
...
---
*Last updated: 2026-08-20 after v1.4 Security Hardening (Phase 11) milestone — 8/8 plans + verification (see .planning/phases/11-security-hardening-audit-remediation-p0-p1/11-VERIFICATION.md); prior milestone v1.3 archived 2026-08-14*
