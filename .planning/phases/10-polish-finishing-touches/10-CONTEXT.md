...
- **D-30:** Projection cache key includes series, effective order, view type, projection version, graph revision, user scope where needed. `graph_revision` is a Redis-local per-series cache epoch (default 0), atomically incremented by existing `invalidate_series` write paths; it is not a Neo4j ontology field. Epoch-read failure bypasses cache. Never cross-return view or boundary.
