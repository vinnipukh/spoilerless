...
- **D-23:** Initial Overview/Character layout uses fCoSE, then stored preset positions. Shared characters remain mostly stable across Episode changes. Evidence uses left-to-right Dagre via locked `cytoscape-dagre@4.0.0` plus its TypeScript declaration package pinned to `2.3.4`; ELK is not added. Timeline uses React/CSS.
...
- **D-29:** Exact read contracts: `GET /api/series/{series_id}/graph/visualization` supports `episode_overview`, `character_network`, `plot_threads`, `investigation`, `full`, `graphrag_focus`; `GET /api/series/{series_id}/graph/expand` enforces boundary, semantic allowlist, and limit server-side. Both use strict neutral DTOs, existing optional-user graph access semantics, required positive `episode_order`, typed 404/422/503 envelopes, and synchronized OpenAPI/frontend-contract inventories. Do not create every conceivable endpoint merely because it appears in design notes.
- **D-30:** Projection cache key includes series, effective order, view type, projection version, graph revision, user scope, and request-specific identity where needed. `graph_revision` is a Redis-local per-series cache epoch (default 0), atomically incremented by existing `invalidate_series` write paths; it is not a Neo4j ontology field. `graphrag_focus` keys include a deterministic digest of validated, deduplicated, sorted focus IDs. Expansion bypasses cache in Phase 10. Epoch-read failure bypasses cache. Never cross-return view, boundary, focus set, or expansion request.
