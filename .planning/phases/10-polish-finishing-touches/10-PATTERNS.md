...
Retain cache-aside semantics: Redis is optional, errors return a miss, values are JSON payloads, writes use TTL, and content-changing writes invalidate conservatively. Extend the key dimensions for view type, projection version, graph revision, effective order, series, and user scope; keep the existing best-effort behavior. A cache hit must never cross view, user, revision, or spoiler boundary.
