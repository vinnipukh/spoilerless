...
- **D-30:** Projection cache key includes series, effective order, view type, projection version, Redis-local per-series cache epoch (`graph_revision`), user scope, and request signature. Existing write invalidation atomically increments epoch; GraphRAG focus signature hashes validated/deduplicated/sorted focus IDs; expansion bypasses cache; epoch-read failure bypasses cache. Never cross-return view, boundary, or focus set.
