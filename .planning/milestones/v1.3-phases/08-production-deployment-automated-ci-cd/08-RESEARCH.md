...
>
> **EXECUTION FINDING (2026-08-04, verified live on the user's AuraDB Free instance — supersedes the Cypher path above):** `CREATE USER` via the Query browser is **denied on AuraDB Free even when connected with the instance admin credential** — console tool-auth connects as a UUID user with the immutable DBMS role `console_admin_free_<dbid>` (which lacks user management on Free), and switching to the credentials-file `neo4j` instance admin still fails with `Neo.ClientError.Security.Forbidden` / `42NFF ... permission/access denied`. Cypher user administration (the docs' Connect-instance Option 1) applies to paid tiers only. **Working paths on Free:** (1) use the instance admin `neo4j` credential from the credentials file as the app's runtime credential (single-credential setup — D-16 least-privilege becomes a documented Free-tier ceiling, the tradeoff is a raw-admin runtime connection per PROBLEMS.md #36), or (2) if an instance-level "Users"/"Access" section exists in the instance dashboard with an Add-user + role dropdown (Administrator/Member/Viewer — NOT Project Settings → Users, which is human console access), create a Member-role DB user there. Verify whichever path before writing DEPLOYMENT.md (DOCS-03).
