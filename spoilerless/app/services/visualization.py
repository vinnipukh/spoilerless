...
# D-21: relation families per expansion key. ``family``/``work``/``conflict``
# expand visible narrative neighbors through these edge types; ``locations``
# expands Location neighbors through the occurrence/location family (which is
# normally omitted from the Episode Overview, D-13 — expansion is the deep
# dive that surfaces it). Every type listed here has a human class mapping in
# FULL_EDGE_CLASSES; any other relationship type fails closed (D-14).
_EXPANSION_EDGE_TYPES: dict[str, frozenset[str]] = {
...
# D-21: expansions only ever add these node kinds. ``locations`` additions are
# Locations, ``episode_events`` additions are Events, ``clues`` additions are
# Claims + Evidence, ``evidence`` additions are Evidence + Sources, and the
# neighbor keys (family/work/conflict) add Characters (the only kind narrative
# edges connect). Anything else cannot appear as an expansion addition.
_EXPANSION_NEIGHBOR_KEYS = (
    EXPANSION_KEY_FAMILY,
    EXPANSION_KEY_WORK,
    EXPANSION_KEY_CONFLICT,
)
...
class VisualizationProjectionService:
    ...
    def project_expansion(
        ...
        The delta is self-contained: the anchor node (required for DTO
        reference closure) plus up to ``limit`` deterministic additions and
        every edge between kept nodes. NO hidden totals/counts exist, the
        allowlist is the only accepted vocabulary (T10-BOUND-06), and the
        hard max of ``EXPANSION_MAX_LIMIT`` additions can never be exceeded
        (T10-LEAK-06). The safe graph was already read at the effective
        boundary; hidden/unknown rows fail closed.

        Key semantics (all additions come from the safe payload only):

        - ``family``/``work``/``conflict``: visible neighbors through the
          FAMILY_OF / WORKS_WITH / conflict relation families.
        - ``locations``: visible Location neighbors through the
          occurrence/location family (normally omitted from the Episode
          Overview, D-13).
        - ``episode_events``: visible Events of the anchored Episode (the
          anchor must be an Episode).
        - ``clues``: visible Claims referencing the anchor plus the Evidence
          those claims cite, with ``supported_by`` edges.
        - ``evidence``: the Evidence for claims referencing the anchor plus
          their Sources, with ``from_source`` edges.

        :raises InvalidVisibilityOrder: hidden/unknown anchor, a non-Episode
            anchor for ``episode_events``, or a hidden investigation row.
        :raises ValueError: an unknown key, a limit outside 1..25, an
            unmapped relationship type, or missing evidence provenance.
        """
        ...
        if anchor is None:
            # Hidden and unknown are indistinguishable by design — the route
            # maps this to a sanitized 422 INVALID_REQUEST.
            raise InvalidVisibilityOrder(
        ...
        # Reveal order of investigation rows (claims/evidence/sources are not
        # graph nodes, so the deterministic addition sort looks their order
        # up here).
        investigation_order: dict[str, int] = {}
        claim_refs: list = []

        if expansion_key in _EXPANSION_NEIGHBOR_KEYS:
            # Narrative neighbors through the key's relation family; the
            # other endpoint must exist and be a Character (the only node
            # kind narrative edges connect, D-21).
            edge_types = _EXPANSION_EDGE_TYPES[expansion_key]
            for edge in graph.edges:
            ...
            edge_types = _EXPANSION_EDGE_TYPES[EXPANSION_KEY_LOCATIONS]
            for edge in graph.edges:
                if edge.type not in edge_types:
                    continue
                other_id = None
            ...
            # Investigation-layer expansion: only claims referencing the
            # anchor (subject or object) are consumed; every consumed
            # investigation row must be visible at the effective boundary
            # (fail closed, like the investigation view).
            for claim in graph.claims:
            ...
            for claim in claim_refs:
                ...
                for evidence_id in claim.evidence_ids:
                    ...
                    # Clues expand claims AND their supporting evidence
                    # (D-21); evidence adds the evidence + its source.
        ...
        # Deterministic bound (D-21/D-22): additions ordered by (reveal
        # order, id), truncated at ``limit`` — never randomly. The anchor is
        # never dropped and never counts against the bound.
        ...
        nodes: list[VisualizationNode] = [self._node(anchor, anchor.type, DISPLAY_TIER_CORE)]
        for nid in addition_ids:
            # Additions mix node kinds: GraphNode (family/work/conflict/
            # locations/episode_events), GraphClaim (clues), and
            # GraphEvidence/GraphSource (evidence). Dispatch by concrete
            # type — claims carry no `type`/`episode_id` media fields.
            node = additions_by_id[nid]
            if isinstance(node, GraphClaim):
                kind = "Claim"
                try:
                    tier = _CLAIM_STATUS_DISPLAY_TIER[node.status]
                except KeyError:
                    raise ValueError(
            ...
            nodes.append(self._node(node, kind, tier))

        # Narrative edges between kept nodes use the FULL human vocabulary
        # (the occurrence/location family surfaces here, D-13/D-14).
        edges: list[VisualizationEdge] = []
        restricted = _EXPANSION_EDGE_TYPES.get(expansion_key)
        for edge in graph.edges:
            if edge.source not in kept_ids or edge.target not in kept_ids:
                continue
            # D-21: a neighbor/location expansion surfaces ONLY its own
            # semantic family — "expand by family" must not leak the
            # KNOWS user-rel or any other edge type between kept nodes.
        ...
        # Investigation-layer expansion edges (claims are not graph edges).
        if expansion_key == EXPANSION_KEY_CLUES:
            for claim in claim_refs:
