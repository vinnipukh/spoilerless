...
| LLM tool calls | Retrieval tools receive the backend-derived **effective** boundary as a server-owned kwarg (`$visible_until_order`, 77 occurrences in `backend/app/retrieval/tools.py`); the LLM can never raise it (D-12). |
...
| **Evidence** | Chain filter: Claim and Evidence both `<= boundary` | `EVIDENCE_QUERY` (filter.py:182); retrieval `EVIDENCE_FOR_CLAIMS_QUERY` / `GET_EVIDENCE_QUERY` (retrieval/tools.py:92,200) | Evidence shown only inside a visible Claim expansion | `backend/tests/test_retrieval_tools.py`, `backend/tests/test_citations.py`; matrix row G4 | A visible Claim must never expose future Evidence (D-11). Evidence hidden if its own or its Claim's order fails. Missing guard → provenance chain leaks. |
