...
- `frontend/.env.example` ships `VITE_API_BASE_URL`, which **no source file reads** (the docs previously claimed both "doesn't exist" and "is used" — it's declared and dead; `frontend/src/api/*.ts` hardcodes `/api`).
