...
Two sibling routes reuse the same spoiler-safe machinery. `POST /api/series/{series_id}/graph/path` executes the allowlisted `find_path` tool with `max_hops` constrained to 1–4. Its request model contains source ID, target ID, and `max_hops` but **no episode boundary**; the current handler passes `MAX_PATH_HOPS` (4) as `requested_order` to `_resolve_effective_boundary`, so authenticated path reads resolve from order 4 before progress clamping and can return `422` for a series without persisted episode order 4. `GET /api/series/{series_id}/export` accepts `visible_until_order` and renders the full visible graph—or one target and its claims—as Markdown from `GraphService.fetch_graph()`. Export therefore follows the shared boundary block as intended; the path route's order-4 coupling is a current implementation limitation, not a second filtering system.
