...
Graph, note, direct custom-content, and revision GET routes require `visible_until_order` as a **required positive integer** identifying a persisted episode order. Candidate list filtering is optional, direct candidate GET has no boundary, and chat session/detail GETs resolve visibility from persisted watch progress instead of this query parameter:
