...
The following table is the complete locked inventory: **21 method/path operations over exactly 14 unique path templates**.
...
No passwords, roles, permissions, account linking, refresh-token Google API access, revisions, soft delete, rich text, uploads, collaboration, LLM/extraction, moderation, queues, vector stores, ontology expansion, ORM, unrelated refactor, or frontend implementation is included here. Canonical fixtures are unchanged.
