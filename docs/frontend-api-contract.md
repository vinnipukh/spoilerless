...
The following table is the complete locked inventory: **39 method/path operations over exactly 28 unique path templates**.
...
`POST /api/series/{series_id}/change-sets` validates every operation server-side (ontology, series scope, current visibility) and persists **only** the `ChangeSet` draft resource itself — no target node, relationship, or claim is ever mutated by this route. Stage 2 (confirm and apply) is a later phase. Request body:
