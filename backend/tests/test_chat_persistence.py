...
from backend.app.repository.chat import ChatRepository
...
def test_no_delete_cypher_targets_chat_message_on_progress_decrease() -> None:
    ...
    ``backend/app/repository/chat.py`` must contain no Cypher ``DELETE``
    ...
    (``CHAT_SESSION_DELETE_QUERY`` in ``backend/app/graph/chat.py``), which
    is itself never invoked by any progress-decrease code path.
    """
    import inspect

    from backend.app.graph import chat as chat_queries
    from backend.app.repository import chat as chat_repository_module
